...
class FinalTestActivity : AppCompatActivity() {
    ...
    private var finalTestResultList: ArrayList<HashMap<String, String>> =
    ...
    override fun onStop() {
        databaseReference =
            firebaseDatabase.getReference("ProjectList").child(PID.toString()).child("finalTest")
                .child("result")
    ...
    private fun recyclerviewInit() {
        ...
        memberNameList = ArrayList()
        myAdapter =
            FinalTestMemberListAdapter(memberNameList, this, final_test_date.text.toString())
...
//        final_test_date.text = ""+calendar.get(Calendar.YEAR)+" / "+(calendar.get((Calendar.MONTH))+1)+" / "+calendar.get(Calendar.DAY_OF_MONTH) // default는 오늘날짜
                    ...
                    DialogInterface.OnClickListener { dialog, which ->
                        ...
                        databaseReference =
                            firebaseDatabase.getReference("ProjectList").child(PID.toString())
                                .child("finalTest").child("test_date")
    ...
    private fun getResultFromDB() {
        ...
        val resultScoreList: HashMap<String, HashMap<String, Double>> = HashMap()
            ...
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                for (snapshot in dataSnapshot.children) { // 누가했는지
                    for (resultSnapshot in snapshot.children) { // 누구를했는지
                        val resultDTO: FinalTestResultDTO =
                            resultSnapshot.getValue(FinalTestResultDTO::class.java)!!
                        val scoreStorage: HashMap<String, Double> =
                            resultScoreList[resultSnapshot.key]!!
                        scoreStorage["result1"] =
                            scoreStorage["result1"]!! + (resultDTO.result1).toDouble()
                        scoreStorage["result2"] =
                            scoreStorage["result2"]!! + (resultDTO.result2).toDouble()
                        scoreStorage["result3"] =
                            scoreStorage["result3"]!! + (resultDTO.result3).toDouble()
                        scoreStorage["result4"] =
                            scoreStorage["result4"]!! + (resultDTO.result4).toDouble()
                ...
                val finalRestResultDialog = FinalTestResultDialog(
                    tmp,
                    FirebaseAuth.getInstance().currentUser!!.displayName.toString()
                )
                val totalScore: Double =
                    myResultScore["result1"]!! + myResultScore["result2"]!! + myResultScore["result3"]!! + myResultScore["result4"]!!
            ...
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                for (snapshot in dataSnapshot.children) {
                    if (snapshot.key == "test_date") {
                        val tmp: FinalTestDateDTO =
                            snapshot.getValue(FinalTestDateDTO::class.java)!!
...
//            final_test_send_result_button.isEnabled = true
            ...
            builder.setPositiveButton("예",
                DialogInterface.OnClickListener { dialog, which ->
                    ...
                    for (i in finalTestResultList.indices) {
                        ...
                        databaseReference =
                            firebaseDatabase.getReference("ProjectList").child(PID.toString())
                                .child("finalTest").child("result").child(myUID)
                                .child(tmpResult["memberUID"]!!)
                        databaseReference.setValue(ftDTO)
                    }
                    final_test_send_result_button.visibility = View.INVISIBLE
                    Toast.makeText(
                        this,
                        "전송이 완료되었습니다. \n모든 팀원이 완료하면 결과를 확인할 수 있습니다.",
                        Toast.LENGTH_SHORT
                    ).show()
                    final_test_show_result_button.setBackgroundResource(R.drawable.button2)
                    final_test_show_result_button.visibility = View.VISIBLE
    ...
    private fun findMembersUIDFromDB() {
        databaseReference =
            firebaseDatabase.getReference("ProjectList").child(PID.toString()).child("members")
        databaseReference.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
